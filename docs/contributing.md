...
3) Keep warnings clean (`-Wall -Wextra -Wpedantic`, `/W4`); enable `PIKA_STRICT_WARNINGS` (default ON).
...
- Renderer gated by `PIKA_ENABLE_RENDERER`; avoid hard deps when OFF.
...
- Run sample executables in `build/examples/...`; editor at `build/editor/pikaeditor`.

## Plugins & Scripts
- Plugins export `createPlugin()` returning `std::shared_ptr<IModule>`.
- Python bridge requires `PIKA_ENABLE_PYTHON=ON` and Python dev headers.
